...
if gh run watch "$RUN_ID" --exit-status; then
  osascript -e 'display notification "Deploy succeeded!" with title "Fulbito" sound name "Glass"'
  echo "Deploy complete!"
else
  osascript -e 'display notification "Deploy FAILED" with title "Fulbito" sound name "Basso"'
